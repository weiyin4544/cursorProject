<template>
  <div class="native-group-members">
    <div class="operation-bar">
      <el-button type="primary" @click="handleAddMembers">添加终端</el-button>
...
import {
  getTerminalsByNativeGroupId,
  addTerminalToNativeGroup,
  removeTerminalFromNativeGroup,
  batchAddTerminalsToNativeGroup
} from '@/api/group'
import { getAllTerminals } from '@/api/terminal'
...
async function loadTableData() {
  loading.value = true
  try {
    const res = await getTerminalsByNativeGroupId(props.groupId)
...
async function confirmAddMembers() {
  if (selectedSearchRows.value.length === 0) {
    ElMessage.warning('请选择要添加的终端')
    return
  }
  
  try {
    const terminalIds = selectedSearchRows.value.map(row => row.id)
    await batchAddTerminalsToNativeGroup(props.groupId, terminalIds)
    ElMessage.success('添加成功')
    addMembersDialogVisible.value = false
    loadTableData()
  } catch (error) {
    console.error('添加终端失败', error)
    ElMessage.error('添加终端失败')
  }
}
...
async function handleRemove(row: any) {
  try {
    await ElMessageBox.confirm('确定要移除该终端吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    await removeTerminalFromNativeGroup(props.groupId, row.id)
...
async function handleBatchRemove() {
  if (selectedRows.value.length === 0) {
    ElMessage.warning('请选择要移除的终端')
    return
  }
  
  try {
    await ElMessageBox.confirm(`确定要移除选中的 ${selectedRows.value.length} 个终端吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    const promises = selectedRows.value.map(row => removeTerminalFromNativeGroup(props.groupId, row.id))
